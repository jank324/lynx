...
def test_quadrupole_off():
    ...
    """
    quadrupole = Quadrupole(length=jnp.asarray(1.0), k1=jnp.asarray(0.0))
    ...
    quadrupole.k1 = jnp.asarray(1.0, device=quadrupole.k1.device)
    outbeam_quad_on = quadrupole(incoming_beam)

    assert jnp.allclose(outbeam_quad.sigma_x, outbeam_drift.sigma_x)
    assert not jnp.allclose(outbeam_quad_on.sigma_x, outbeam_drift.sigma_x)
...
def test_quadrupole_with_misalignments_vectorized():
    ...
    quad_with_misalignment = Quadrupole(
        ...
        k1=jnp.asarray(1.0),
        misalignment=jnp.asarray([0.1, 0.1]).unsqueeze(0),
    )

    quad_without_misalignment = Quadrupole(length=jnp.asarray(1.0), k1=jnp.asarray(1.0))
    incoming_beam = ParameterBeam.from_parameters(
        sigma_px=jnp.asarray(2e-7), sigma_py=jnp.asarray(2e-7)
    )
...
def test_quadrupole_with_misalignments_multiple_vector_dimensions():
    ...
    quad_with_misalignment = Quadrupole(
        length=jnp.asarray(1.0),
        k1=jnp.asarray(1.0),
        misalignment=jnp.randn((4, 3, 2)) * 5e-4,
    )
    quad_without_misalignment = Quadrupole(length=jnp.asarray(1.0), k1=jnp.asarray(1.0))
    ...
    segment = Segment(
        [
            Quadrupole(
                ...
                k1=jnp.asarray(1.0),
                tilt=jnp.asarray([jnp.pi / 4, jnp.pi / 2, jnp.pi * 5 / 4]),
            ),
...
def test_tilted_quadrupole_multiple_vector_dimensions():
    ...
    segment = Segment(
        [
            Quadrupole(
                ...
                tilt=jnp.asarray(
                    [
                        [jnp.pi / 4, jnp.pi / 2, jnp.pi * 5 / 4],
                        [jnp.pi * 5 / 4, jnp.pi / 2, jnp.pi / 4],
                    ]
                ),
            ),
            Drift(length=jnp.asarray(0.5)),
    ...
    assert jnp.allclose(outgoing.particles[0, 0], outgoing.particles[1, 2])
    assert jnp.allclose(outgoing.particles[0, 1], outgoing.particles[1, 1])
    assert jnp.allclose(outgoing.particles[0, 2], outgoing.particles[1, 0])
...
def test_quadrupole_length_multiple_vector_dimensions():
    ...
    not raise an error and behaves as expected.
    """
    lengths = jnp.asarray([[0.2, 0.3, 0.4], [0.5, 0.4, 0.7]])
    segment = Segment(
        [
            Quadrupole(length=lengths, k1=jnp.asarray(4.2)),
    ...
    assert outgoing.particles.shape == (2, 3, 10_000, 7)
    assert jnp.allclose(outgoing.particles[0, 2], outgoing.particles[1, 1])
...
def test_quadrupole_bmadx_tracking(dtype):
    ...
    quadrupole = Quadrupole(
        length=jnp.asarray(1.0),
        k1=jnp.asarray(10.0),
        misalignment=jnp.asarray([0.01, -0.02], dtype=dtype),
        tilt=jnp.asarray(0.5),
        num_steps=10,
    ...
    assert jnp.allclose(
        ...
        atol=1e-14 if dtype == jnp.float64 else 1e-5,
        rtol=1e-14 if dtype == jnp.float64 else 1e-6,
    )


@pytest.mark.parametrize("tracking_method", ["lynx", "bmadx"])
def test_tracking_method_vectorization(tracking_method):
    ...
    """
    quadrupole = Quadrupole(
        length=jnp.asarray([[0.2, 0.25], [0.3, 0.35], [0.4, 0.45]]),
        k1=jnp.asarray([[4.2, 4.2], [4.3, 4.3], [4.4, 4.4]]),
        misalignment=jnp.zeros(2),
        tilt=jnp.asarray(0.0),
