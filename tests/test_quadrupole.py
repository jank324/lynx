...
def test_quadrupole_off():
    ...
    """
    quadrupole = Quadrupole(length=jnp.array([1.0]), k1=jnp.array([0.0]))
    ...
    quadrupole.k1 = jnp.array([1.0], device=quadrupole.k1.device)
...
def test_quadrupole_with_misalignments_batched():
    ...
    quad_with_misalignment = Quadrupole(
        ...
        k1=jnp.array([1.0]),
        misalignment=jnp.array([[0.1, 0.1]]),
    )

    quad_without_misalignment = Quadrupole(length=jnp.array([1.0]), k1=jnp.array([1.0]))
    incoming_beam = ParameterBeam.from_parameters(
        sigma_xp=jnp.array([2e-7]), sigma_yp=jnp.array([2e-7])
    )
...
def test_quadrupole_with_misalignments_multiple_batch_dimension():
    ...
    quad_without_misalignment = Quadrupole(
        length=jnp.array([1.0]), k1=jnp.array([1.0])
    ).broadcast(batch_shape)
    incoming_beam = ParameterBeam.from_parameters(
        sigma_xp=jnp.array([2e-7]), sigma_yp=jnp.array([2e-7])
    ).broadcast(batch_shape)
    ...
    segment = Segment(
        [
            Quadrupole(
                ...
                tilt=jnp.array([jnp.pi / 4, jnp.pi / 2, jnp.pi * 5 / 4]),
            ),
    ...
    segment = Segment(
        [
            Quadrupole(
                ...
                k1=jnp.array([1.0]),
                tilt=jnp.array([jnp.pi / 4]),
            ),
            Drift(length=jnp.array([0.5])),
        ]
    ).broadcast(batch_shape)
    outgoing = segment(incoming)

    assert jnp.allclose(outgoing.particles[0, 0], outgoing.particles[0, 1])
