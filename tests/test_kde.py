...
from lynx.utils import kde_histogram_1d, kde_histogram_2d
...
def test_weighted_samples_1d():
    ...
    weighted samples.
    """
    x_unweighted = jnp.asarray([1.0, 1.0, 1.0, 2.0])
    x_weighted = jnp.asarray([1.0, 2.0])

    bins = jnp.linspace(0, 3, 10)
    sigma = jnp.asarray(0.3)
    ...
    hist_weighted = kde_histogram_1d(
        x_weighted, bins, sigma, weights=jnp.asarray([3.0, 1.0])
    )
    # Use samples but neglect the weights
    hist_neglect_weights = kde_histogram_1d(x_weighted, bins, sigma)

    assert jnp.allclose(hist_unweighted, hist_weighted)
    assert not jnp.allclose(hist_weighted, hist_neglect_weights)
...
def test_weighted_samples_2d():
    ...
    weighted samples.
    """
    x_unweighted = jnp.asarray([[1.0, 2.0], [1.0, 2.0], [1.0, 2.0], [2.0, 1.0]])
    x_weighted = jnp.asarray([[1.0, 2.0], [2.0, 1.0]])

    bins1 = jnp.linspace(0, 3, 10)
    bins2 = jnp.linspace(0, 3, 10)
    sigma = jnp.asarray(0.3)
...
def test_kde_1d_basic_usage():
    ...
    """
    data = jnp.randn((5, 100))  # 5 beamline states, 100 particles in 1D
    bins = jnp.linspace(0, 1, 10)  # A single histogram
    sigma = jnp.asarray(0.1)  # A single bandwidth
...
def test_kde_1d_enforce_bins_shape():
    ...
    with pytest.raises(ValueError):
        kde_histogram_1d(data, bins, jnp.rand(3) + 0.1)
...
def test_kde_2d_vectorized_basic_usage():
    ...
    data = jnp.randn((3, 2, 100, 6))
    # Two different bins (1 per path)
    num_bins = 30
    bins_x = jnp.linspace(-20, 20, num_bins)
    # A single bandwidth
    sigma = jnp.asarray(0.1)
