...
def convert_element(name: str, context: dict) -> "lynx.Element":
    ...
    if isinstance(bmad_parsed, list):
        ...
        if bmad_parsed["element_type"] == "marker":
            ...
            if "l" in bmad_parsed:
                return lynx.Drift(length=jnp.array([bmad_parsed["l"]]), name=name)
            else:
                return lynx.Marker(name=name)
            ...
            return lynx.HorizontalCorrector(
                length=jnp.array([bmad_parsed.get("l", 0.0)]),
                angle=jnp.array([bmad_parsed.get("kick", 0.0)]),
                name=name,
            )
            ...
            return lynx.Dipole(
                length=jnp.array([bmad_parsed["l"]]),
                gap=jnp.array([bmad_parsed.get("hgap", 0.0)]),
                angle=jnp.array([bmad_parsed.get("angle", 0.0)]),
                e1=jnp.array([bmad_parsed["e1"]]),
                e2=jnp.array([bmad_parsed.get("e2", 0.0)]),
                tilt=jnp.array([bmad_parsed.get("ref_tilt", 0.0)]),
                fringe_integral=jnp.array([bmad_parsed.get("fint", 0.0)]),
                fringe_integral_exit=(
                    jnp.array([bmad_parsed["fintx"]])
            ...
            return lynx.Quadrupole(
                length=jnp.array([bmad_parsed["l"]]),
                k1=jnp.array([bmad_parsed["k1"]]),
                tilt=jnp.array([bmad_parsed.get("tilt", 0.0)]),
                name=name,
            )
            ...
            return lynx.Cavity(
                length=jnp.array([bmad_parsed["l"]]),
                voltage=jnp.array([bmad_parsed.get("voltage", 0.0)]),
                phase=jnp.array(
                    [-jnp.degrees(bmad_parsed.get("phi0", 0.0) * 2 * jnp.pi)]
                ),
                frequency=jnp.array([bmad_parsed["rf_frequency"]]),
                name=name,
            )
            ...
            return lynx.Aperture(
                x_max=jnp.array([bmad_parsed.get("x_limit", jnp.inf)]),
                y_max=jnp.array([bmad_parsed.get("y_limit", jnp.inf)]),
                shape="rectangular",
                name=name,
            )
            ...
            return lynx.Undulator(length=jnp.array([bmad_parsed["l"]]), name=name)
            ...
            return lynx.Drift(length=jnp.array([bmad_parsed.get("l", 0.0)]), name=name)
        else:
            print(
            ...
            return lynx.Drift(name=name, length=jnp.array([bmad_parsed.get("l", 0.0)]))
    else:
